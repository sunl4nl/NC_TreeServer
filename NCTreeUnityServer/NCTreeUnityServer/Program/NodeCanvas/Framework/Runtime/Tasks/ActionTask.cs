	...
	abstract public class ActionTask : Task{
...
#if SERVER_UNITY
        Component agentRef;
        IBlackboard blackboardRef;
        Action<bool> callbackRef;
#endif
        ///Used to call an action providing a callback.
        public void ExecuteAction(Component agent, IBlackboard blackboard, Action<bool> callback){
			if (!isRunning){
                ...
                MonoManager.current.onUpdate += OnMonoUpdate;
#else
        ...
        void OnMonoUpdate()
        {
            if (ExecuteAction(agentRef, blackboardRef) != Status.Running)
            {
                if (callbackRef != null)
                {
                    callbackRef(status == Status.Success ? true : false);
                }
            }
        }
#else
