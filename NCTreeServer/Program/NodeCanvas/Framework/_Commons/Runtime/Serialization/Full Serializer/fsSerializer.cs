...
*/
        ...
        private static void Invoke_OnBeforeSerialize(List<fsObjectProcessor> processors, Type storageType, object instance) {
            ...
            if (instance is UnityEngine.ISerializationCallbackReceiver && !(instance is Object) ){
...
#if ABSTRACT_UNITY
            if (instance is AbsUnity.ISerializationCallbackReceiver && !(instance is Object))
            {
                ((AbsUnity.ISerializationCallbackReceiver)instance).OnAfterDeserialize();
            }
#else
            //PARADOXNOTION ADDITION
            //!!Call only on non-Unity objects, since they are called back anyways by Unity!!
